...
volatile bool UDPManager::connected_ = false;
volatile TickType_t UDPManager::last_alive_tick_ = 0;
...
#define FLAG_STOP_STREAM    0x0003
#define FLAG_START_STREAM   0x0004
#define FLAG_PACKET_LOSS    0x0005
#define FLAG_ALIVE          0x0006
...
esp_err_t UDPManager::connect() {
    if (!initialized_) {
        ESP_LOGE(TAG, "UDPManager::connect() called before init");
        return ESP_ERR_INVALID_STATE;
    }

    for (int attempt = 0; attempt < UDP_HANDSHAKE_MAX_RETRIES; attempt++) {

        ESP_LOGI(TAG, "Sending handshake attempt %d/%d", attempt + 1, UDP_HANDSHAKE_MAX_RETRIES);
        esp_err_t ret = sendHandshake();

        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to send handshake: %s", esp_err_to_name(ret));
            return ret;
        }

        // wait for ACK — receive task will set connected_ flag
        TickType_t elapsed = 0;
        while (elapsed < pdMS_TO_TICKS(UDP_HANDSHAKE_PING_INTERVAL_MS)) {
            if (connected_) {
                connected_ = false;
                last_alive_tick_ = xTaskGetTickCount();
                ESP_LOGI(TAG, "Handshake ACK received — connected");
                return ESP_OK;
            }
            vTaskDelay(pdMS_TO_TICKS(10));
            elapsed += pdMS_TO_TICKS(10);
        }

        ESP_LOGW(TAG, "Handshake attempt %d timed out", attempt + 1);
    }

    ESP_LOGE(TAG, "Handshake failed after %d attempts", UDP_HANDSHAKE_MAX_RETRIES);
...
esp_err_t UDPManager::deinit() {
    ...
    while (recv_task_handle_ != nullptr) {
        if (elapsed >= TASK_STOP_TIMEOUT_MS) {
            ESP_LOGE(TAG, "deinit timed out waiting for receive task");
            return ESP_ERR_TIMEOUT;
        }
        vTaskDelay(TASK_STOP_POLL_MS);
        elapsed += TASK_STOP_POLL_MS;
    }
...
void UDPManager::udpReceiveTask(void* param) {
    ...
    while (initialized_) {
        ...
        switch (flags) {
            case FLAG_HANDSHAKE_ACK:
                ESP_LOGI(TAG, "Handshake ACK received");
		connected_ = true;
		last_alive_tick_ = xTaskGetTickCount();
                if (event_callback_) event_callback_(UDPEvent::HANDSHAKE_ACK);
                break;
            case FLAG_ALIVE:
                last_alive_tick_ = xTaskGetTickCount();
                break;
            case FLAG_START_STREAM:
                ESP_LOGI(TAG, "START_STREAM received from server");
                if (event_callback_) event_callback_(UDPEvent::START_STREAM);
                break;
            case FLAG_STOP_STREAM:
                ESP_LOGI(TAG, "STOP_STREAM received from server");
                if (event_callback_) event_callback_(UDPEvent::STOP_STREAM);
                break;
            case FLAG_PACKET_LOSS: {
                uint8_t loss_pct = (len > UDP_HEADER_SIZE) ? rx_buf[UDP_HEADER_SIZE] : 0;
                ESP_LOGW(TAG, "Packet loss report: %u%%", loss_pct);
                if (event_callback_) event_callback_(UDPEvent::PACKET_LOSS);
                break;
            }
            default:
                ESP_LOGW(TAG, "Unknown flag received: 0x%04X", flags);
                break;
        }

    	if (connected_) {
    	    TickType_t now = xTaskGetTickCount();
    	    if ((now - last_alive_tick_) >= pdMS_TO_TICKS(UDP_ALIVE_TIMEOUT_MS)) {
    	        ESP_LOGW(TAG, "Alive signal lost — connection dropped");
    	        connected_ = false;
    	        if (event_callback_) event_callback_(UDPEvent::CONNECTION_LOSS);
    	    }
    	}
    }
...
bool UDPManager::isStreaming()    { return streaming_; }
bool UDPManager::isInitialized()  { return initialized_; }
